...
<details>
<summary><strong>Приложение меняет мой hosts?</strong></summary>
Да, пока обход работает: в hosts добавляется блок с адресами голосовых серверов Discord — без него голос не подключается. Блок помечен маркерами, перед перезаписью создаётся резервная копия <code>hosts.unblockpro.bak</code>, а ваши собственные записи не трогаются.

При отключении блок убирается: hosts имеет приоритет над DNS, поэтому оставленные адреса ломали бы Discord даже с выключенным обходом и под VPN. Деинсталлятор Windows тоже чистит блок. Если записи остались от старой версии — кнопка «Убрать записи из hosts» в главном окне.
</details>
...
Попробуйте отключиться и подключиться заново — приложение переберёт другие стратегии. Если ни одна не помогла, возможно, ваш провайдер использует продвинутый DPI — создайте Issue и приложите журнал: разверните карточку «Журнал» в главном окне и нажмите «Скопировать журнал».
</details>

<details>
<summary><strong>Что значит «Защита частичная»</strong></summary>
Приложение проверяет каждую стратегию отдельно по YouTube и по Discord. Если ни одна стратегия не открыла оба сервиса, но какая-то открыла один из них — подключение выполняется по ней, а недоступный сервис показывается зачёркнутым.

Так сделано намеренно: на части провайдеров Discord не поднимается ни одной стратегией, и раньше в этом случае отвергались вообще все стратегии, включая те, что полностью чинили YouTube. Лучше рабочий YouTube и честная пометка про Discord, чем «ни одна стратегия не сработала» и ничего.
</details>
...
Перед отправкой проверьте отчёт и удалите личные сведения. Строка
`AMFI: Denying core dump` в системной консоли сама по себе не определяет причину.

Проверка интерфейса без запуска обхода и изменения системных настроек:

```bash
npx electron scripts/renderer-smoke.js
```
